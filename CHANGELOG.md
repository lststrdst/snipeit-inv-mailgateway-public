...
- Я уточнил метку подтверждённой смены владельца: `[OWNER CHANGE]`;
  `[RECOVERY]` обозначает восстановление компонента после ошибки.
- Я добавил HTML/PNG-примеры писем `OWNER CHANGE` и `REPORT` с вымышленными
  данными и локальным генератором на основе действующих шаблонов.
- Я сохранил публичную подпись автора только в корневом README; проверка
  анонимизации по-прежнему блокирует внутренние данные и учётные имена.
